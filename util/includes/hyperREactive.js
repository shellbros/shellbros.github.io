...
export class HyperElement {
	constructor () {
		...
		if (arguments[i] && arguments[i][isProxy]) {
			this.storeObject = storeMap.get(arguments[i])
			this.bindings = bindingsMap.get(arguments[i])
			i++
		}
		...
		while (arguments[i] && !(arguments[i] instanceof HyperElement)) {
			...
			if (arg.constructor.name === 'String') {
				...
				Object.keys(arg).forEach(k => {
					if (k.startsWith('on')) {
						let fn = arg[k]
						this.dom.addEventListener(k.slice(2), e => fn(e, this))
					}
					else {
						switch (k) {
							case 'style':
 ...
 * @param {...HyperElement|HyperElement[]} [chilrdren] - List of child HyperElements.
 */
